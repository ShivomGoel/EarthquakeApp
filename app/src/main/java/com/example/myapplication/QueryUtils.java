...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONObject jsonstr = new JSONObject(SAMPLE_JSON_RESPONSE);
            JSONArray jsonftr = jsonstr.getJSONArray("features");

            for(int i = 0; i < jsonftr.length() ; i++){

                JSONObject earthquake = jsonftr.getJSONObject(i);
                JSONObject jsonprop = earthquake.getJSONObject("properties");
                double magnitude = jsonprop.getDouble("mag");
                String place = jsonprop.getString("place");
                long time = jsonprop.getLong("time");
                String url = jsonprop.getString("url");
                Earthquake temp = new Earthquake(magnitude,place,time,url);
